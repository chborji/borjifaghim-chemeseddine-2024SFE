                            ...
                            <li class="breadcrumb-item">
                                <a class="breadcrumb-link"
                                   href="{{route('branch.orders.list',['status'=>'all'])}}">
                                    {{\App\CentralLogics\translate('Commandes')}}
                                </a>
                            </li>
                            <li class="breadcrumb-item active"
                                aria-current="page">{{\App\CentralLogics\translate('Détails de la ')}}   {{\App\CentralLogics\translate('commande')}}</li>
                        </ol>
                    </nav>
...
{{--                        </a>--}}
                        ...
                        <span target="_blank" type="button"
                                ...
                                class="tio-print mr-1"></i> {{\App\CentralLogics\translate(' Imprimer la')}} {{\App\CentralLogics\translate(' facture')}}</span>
                            ...
                            @if($order['order_type'] != 'pos')
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.status',['id'=>$order['id'],'order_status'=>'pending'])}}','Change status to pending ?')"href="javascript:">{{\App\CentralLogics\translate('pending')}}</a>
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.status',['id'=>$order['id'],'order_status'=>'confirmed'])}}','Change status to confirmed ?')"href="javascript:">{{\App\CentralLogics\translate('confirmed')}}</a>
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.status',['id'=>$order['id'],'order_status'=>'processing'])}}','Change status to processing ?')"href="javascript:">{{\App\CentralLogics\translate('processing')}}</a>
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.status',['id'=>$order['id'],'order_status'=>'out_for_delivery'])}}','Change status to out for delivery ?')"href="javascript:">{{\App\CentralLogics\translate('out_for_delivery')}}</a>
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.status',['id'=>$order['id'],'order_status'=>'delivered'])}}','Change status to delivered ?')"href="javascript:">{{\App\CentralLogics\translate('delivered')}}</a>
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.status',['id'=>$order['id'],'order_status'=>'returned'])}}','Change status to returned ?')"href="javascript:">{{\App\CentralLogics\translate('returned')}}</a>
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.status',['id'=>$order['id'],'order_status'=>'failed'])}}','Change status to failed ?')"href="javascript:">{{\App\CentralLogics\translate('failed')}}</a>
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.status',['id'=>$order['id'],'order_status'=>'canceled'])}}','Change status to canceled ?')"href="javascript:">{{\App\CentralLogics\translate('canceled')}}</a>
                                    </div>
                                </div>
                            @endif
                        </div>
                        <div class="hs-unfold float-right pr-2">
                            @if($order['order_type'] != 'pos')
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.payment-status',['id'=>$order['id'],'payment_status'=>'paid'])}}','Change status to paid ?')"href="javascript:">{{\App\CentralLogics\translate('paid')}}</a>
                                        <a class="dropdown-item"onclick="route_alert('{{route('branch.orders.payment-status',['id'=>$order['id'],'payment_status'=>'unpaid'])}}','Change status to unpaid ?')"href="javascript:">{{\App\CentralLogics\translate('unpaid')}}</a>
                                    </div>
                                </div>
                            @endif
                        </div>
                ...
                <div class="col-sm-auto">
                    <a class="btn btn-icon btn-sm btn-ghost-secondary rounded-circle mr-1"
                       ...
                       data-toggle="tooltip" data-placement="top" title="Commande précédente">
                    ...
                    <a class="btn btn-icon btn-sm btn-ghost-secondary rounded-circle"
                       ...
                       data-placement="top" title="Commande suivante">
                ...
                <div class="card mb-3 mb-lg-5">
                        ...
                        <div class="row">
                            <div class="col-12 pb-2 border-bottom">
                                <h4 class="card-header-title">
                                    {{\App\CentralLogics\translate('détails de la ')}} {{\App\CentralLogics\translate('commande')}}
                                    <span
                            ...
                            <div class="col-6 pt-2">
                                <div class="text-right">
                                    <h6 class="text-capitalize" style="color: #8a8a8a;">
                                        {{\App\CentralLogics\translate('Méthode de')}} {{\App\CentralLogics\translate('paiment')}} : {{str_replace('_',' ',$order['payment_method'])}}
                                    </h6>
                    ...
                    @foreach($order->details as $detail)
                        @if($detail->product)
                            ...
                            <!-- Media -->
                                <div class="media">
                                    <div class="avatar avatar-l mr-1">
                        ...
                        <div class="row justify-content-md-end mb-3">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-sm-right">
                                    <dt class="col-sm-6">{{\App\CentralLogics\translate('prix')}}:</dt>
                                    ...
                                    <dt class="col-sm-6">{{\App\CentralLogics\translate('Supplémets')}}:</dt>
                                    <dd class="col-sm-6">
                                    ...
                                    <dd class="col-sm-6">{{$sub_total+$total_tax+$add_ons_cost." ".\App\CentralLogics\Helpers::currency_symbol()}}</dd>
                                </dl>
                                <!-- End Row -->
                            </div>
                        </div>
