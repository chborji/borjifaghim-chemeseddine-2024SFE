    ...
    public function list(Request $request, $status)
    {
        ...
        if ($request->has('search')) {
            ...
            if (session('branch_filter') == 0) {
                if ($status != 'all') {
                    $query = Order::with(['branch'])->where(['order_status' => $status]);
                } else {
                    $query = Order::with(['branch']);
                }
            } else {
                if ($status != 'all') {
                    $query = Order::with(['branch'])->where(['order_status' => $status, 'branch_id' => session('branch_filter')]);
                } else {
                    $query = Order::with(['branch'])->where(['branch_id' => session('branch_filter')]);
                }
            }
        }
 ...
 return view('admin-views.order.list', compact('orders', 'status', 'search'));
    ...
    public function details($id)
    {
        ...
        if (isset($order)) {
            ...
            Toastr::info("Il n'y a plus de commandes !");
            return back();
        }
    }
