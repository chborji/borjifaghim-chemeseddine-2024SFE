...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        try {
            ...
            foreach ($request['cart'] as $c) {
                ...
                elseif ($c['type'] == "formule")
                    $product = Formule::find($c['product_id']);
                else

                    $product = ProductCompose::find($c['product_id']);
                ...
                $or_d = [
                    ...
                    'formule_details' => json_encode($c['list_choices']),
                    'compose_details' => json_encode($c['listchoices']),
    ...
    public function get_order($id)
    {
        try {
            ...
            return response()->json([
                'errors' => ['code' => 'order-001', 'message' => trans('order.no_data_found')]
            ], 404);
        }
    }
    ...
    public function update_payment_status(Request $request)
    {
        ...
        if (Order::where(['table_id' => $request['table_id']])->first()) {
            Order::where(['table_id' => $request['table_id']])->update([
                'payment_status' => $request['payment_status']
            ]);
        ...
        return response()->json([
            'errors' => [
                ['code' => 'orderjyfjf' + $request['order_id'], 'message' => trans('custom.no_data_found')]
            ]
        ], 401);
    }
}
