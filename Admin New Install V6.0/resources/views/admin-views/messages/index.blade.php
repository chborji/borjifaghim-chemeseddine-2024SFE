                    ...
                    <div class="card-body" style="overflow-y: scroll;height: 600px">
                        ...
                        @foreach($conversations as $conv)
                            @if(in_array($conv->user_id,$array)==false)
                                ...
                                <div
                                    class="d-flex border-bottom pb-2 pt-2 justify-content-between align-items-center customer-list {{$unchecked!=0?'conv-active':''}}"onclick="viewConvs('{{route('admin.message.view',[$conv->user_id])}}','customer-{{$conv->user_id}}')"style="cursor: pointer; padding-left: 6px; border-radius: 10px;margin-top: 2px;"id="customer-{{$conv->user_id}}">
                                    <div class="avatar avatar-lg avatar-circle d-none d-md-block">
                                        <img class="avatar-img" style="width: 54px;height: 54px"src="{{asset('storage/app/public/profile/'.$user['image'])}}" alt="Image Description">
                                    </div>
                                    <h5 class="mb-0 mr-3">
