...
@section('title','Ajouter produit composé')
    ...
    <div class="page-header">
        <div class="row align-items-center">
            <div class="col-sm mb-2 mb-sm-0">
                <h1 class="page-header-title"><i class="tio-add-circle-outlined"></i> {{\App\CentralLogics\translate('ajouter')}} {{\App\CentralLogics\translate('produit')}} {{\App\CentralLogics\translate('composé')}}</h1>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-6">
            <div class="form-group lang_form" id="">
                ...
                <input type="text" id="product-name" name="name[]" class="form-control" placeholder="produit" required>
            </div>
            <div class="form-group">
                <label class="input-label" for="exampleFormControlInput1">{{\App\CentralLogics\translate('prix')}}</label>
                <input type="number" id="product-price" min="0" step="0.01" name="price" class="form-control" placeholder="Ex : 10 TND" required>
            </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-12 from_part_2">
                        <div class="form-group">
                            ...
                            <img style="width: 40%;border: 1px solid; border-radius: 10px;" id="viewer" src="{{ asset('assets/admin/img/900x400/img1.jpg') }}" alt="image" />
...
<script>
    ...
    $('#product_form').on('submit', function() {
        ...
        formData.append("ingredients", JSON.stringify(allIngredinat))
        let data = {
            name: productName,
            price: productPrice,
        ...
        $.ajax({
            url: "{{route('admin.productcompose.ajouter')}}",
            data: formData,
            type: "POST",
            cache: false,
            contentType: false,
...
<!-- <div class="row gx-2 gx-lg-3">
        <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
            <form action="{{route('admin.productcompose.store')}}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    <div class="form-group col-6">
                        ...
                        <div class="form-group {{ $lang != $default_lang ? 'd-none' : '' }} lang_form" id="{{ $lang }}-form">
                            <label class="input-label" for="exampleFormControlInput1">{{ \App\CentralLogics\translate('name') }}
                            </label>
                            <input type="text" name="name[]" class="form-control" placeholder="New ingredient" {{ $lang == $default_lang ? 'required' : '' }}>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group lang_form" id="{{ $default_lang }}-form">
                                    ...
                                    <input type="text" name="name[]" class="form-control" placeholder="New ingredient" required>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" min="0" step="0.01" name="price" class="form-control" placeholder="Ex : 100" required>
                                </div>
                            </div>
                        </div>

                        <div class="form-group">
                            <input type="button" name="bouton" value="ajouter ingredient" onclick="ajouter()" class="btn btn-primary" />
                        </div>
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group">
                                    <label class="input-label" for="exampleFormControlInput1">{{\App\CentralLogics\translate('requis')}}</label>
                                    <input type="number" min="0" name="requis" class="form-control" placeholder="Ex : 2" required>
                                </div>
                            </div>
                        </div>
                        <div class="row col-6" style="border: 1px solid #80808045; border-radius: 10px;padding-top: 10px;margin: 1px">
                            <div class="form-group col-6">
                                <div class="">
                                    <label class="input-label" for="exampleFormControlSelect1">{{\App\CentralLogics\translate('category')}}<span class="input-label-secondary">*</span></label>
                                    <select name="category_id" class="form-control js-select2-custom" onchange="getRequest('{{url('/')}}/admin/ingredient/get-categories?parent_id='+this.value,'ingredients' )" multiple="multiple" required>
                                        <option value="">---{{\App\CentralLogics\translate('select')}}---</option>
                                        @foreach($ingcategories as $ingcategory)
                                        <option value="{{$ingcategory['id']}}">{{$ingcategory['name']}}</option>
                                        @endforeach
                                    ...
                                    <div class="form-group">
                                        <label class="input-label" for="exampleFormControlSelect1">{{\App\CentralLogics\translate('ingredient')}}<span class="input-label-secondary"></span></label>
                        ...
                        <div class="form-group col-6 form_part_2">
                            <hr>
                            <div class="">
                                <label>{{\App\CentralLogics\translate('image')}}</label><small style="color: red">* ( {{\App\CentralLogics\translate('ratio')}} 1:1 )</small>
                                <div class="custom-file">
