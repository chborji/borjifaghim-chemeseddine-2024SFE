...
<div class="content container-fluid">
    ...
    <div class="page-header">
        <div class="media mb-3">
            ...
            <div class="avatar avatar-xl avatar-4by3 mr-2">
                <img class="avatar-img" src="{{asset('assets/admin')}}/svg/illustrations/earnings.png" alt="Image Description">
            </div>
            <!-- End Avatar -->
            ...
            <div class="media-body">
                <div class="row">
                    <div class="col-lg mb-3 mb-lg-0 text-capitalize">
                        <h1 class="page-header-title">{{\App\CentralLogics\translate('rapport')}} {{\App\CentralLogics\translate("d'activité")}} </h1>
    ...
    <div class="row border-bottom border-right border-left border-top" style="border-radius: 10px">
        ...
        $to = session('to_date');
        $total_tax=\App\Model\Order::where(['order_status'=>'confirmed'])
        ...
        <div class="col-sm-6 col-lg-6 mb-3 mb-lg-6">
            @php
            $total_sold=\App\Model\Order::where(['order_status'=>'confirmed'])->whereBetween('created_at', [$from, $to])->sum('order_amount');
            if($total_sold==0){
            $total_sold=.01;
            }
            @endphp
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            <div class="media">
                                ...
                                <div class="media-body">
                                    <h4 class="mb-1">{{\App\CentralLogics\translate('total')}} des {{\App\CentralLogics\translate('ventes')}}</h4>
    ...
    <div class="card mb-3 mb-lg-5 border-top border-left border-right border-bottom">
        ...
        <div class="card-header">
            @php
            $total_sold=\App\Model\Order::where(['order_status'=>'confirmed'])->whereBetween('created_at', [date('y-01-01'), date('y-12-31')])->sum('order_amount')
            @endphp
            <h6 class="card-subtitle mb-0">Total sale of {{date('Y')}} :<span class="h3 ml-sm-2"> {{$total_sold." ".\App\CentralLogics\Helpers::currency_symbol()}}</span>
            </h6>
        ...
        @php
        $sold=[];
        for ($i=1;$i<=12;$i++){ $from=date('Y-'.$i.'-01'); $to=date('Y-'.$i.'-30'); $sold[$i]=\App\Model\Order::where(['order_status'=>'confirmed'])->whereBetween('created_at', [$from, $to])->sum('order_amount');
            }
            @endphp

            @php
            $tax=[];
            for ($i=1;$i<=12;$i++){ $from=date('Y-'.$i.'-01'); $to=date('Y-'.$i.'-30'); $tax[$i]=\App\Model\Order::where(['order_status'=>'confirmed'])->whereBetween('created_at', [$from, $to])->sum('total_tax_amount');
                }
                @endphp
